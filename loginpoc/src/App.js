...
function App() {
  ...
  const insertExample = e => postTextInput.value = "https://www.reddit.com/r/news/comments/ndyvkx/now_is_not_the_time_nurses_union_condemns_cdc_for/?utm_source=share&utm_medium=web2x&context=3"

  return (
    <div className="card">
        <h1>Reddit Login Test Page</h1>
        <form onSubmit={getRedditPostData}>
            <input type="text" id="post" placeholder="Insert Reddit Post ID or URL"/>
            <input type="submit" value="Get Post Details" id="submitButton"/>
        </form>
        <input type="button" value="Insert example url" id="exampleButton" onSubmit={insertExample}/>
        <br/>
        <br/>
        <Login/>
    </div>
